...
class AgIntegrationService:
    ...
    async def list_integrations_service(cls, auth: AuthSchema, search: AgIntegrationQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgIntegrationQueryParam | None - 查询参数
        ...
        obj_list = await AgIntegrationCRUD(auth).list_integrations_crud(search=search_dict, order_by=order_by)
        return [AgIntegrationOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_integrations_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgIntegrationQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgIntegrationCRUD(auth).page_integrations_crud(
            offset=offset,
            limit=page_size,
    ...
    async def create_integrations_service(cls, auth: AuthSchema, data: AgIntegrationCreateSchema) -> dict:
        ...
        - dict - 创建结果
        """
        obj = await AgIntegrationCRUD(auth).create_integrations_crud(data=data)
        return AgIntegrationOutSchema.model_validate(obj).model_dump()
    ...
    async def set_available_integrations_service(cls, auth: AuthSchema, data: BatchSetAvailable) -> None:
        ...
        - None
        """
        await AgIntegrationCRUD(auth).set_available_integrations_crud(ids=data.ids, status=data.status)
    ...
    async def batch_export_integrations_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'uuid': '',
            'name': '渠道名称',
            'type': '渠道类型(slack/telegram/whatsapp/agui/discord)',
            'agent_id': '绑定AgentID（三选一）',
            'team_id': '绑定TeamID（三选一）',
            'workflow_id': '绑定WorkflowID（三选一）',
            'token': '渠道访问Token（Slack/Telegram等）',
            'signing_secret': '签名密钥（Slack校验用）',
            'prefix': '路由前缀（如/slack /telegram）',
            'config': '渠道扩展配置（streaming/reply_to_mentions_only等）',
            'status': '',
            'description': '',
            'created_time': '',
            'updated_time': '',
    ...
    @classmethod
    async def batch_import_integrations_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '': 'uuid',
            '渠道名称': 'name',
            '渠道类型(slack/telegram/whatsapp/agui/discord)': 'type',
            '绑定AgentID（三选一）': 'agent_id',
            '绑定TeamID（三选一）': 'team_id',
            '绑定WorkflowID（三选一）': 'workflow_id',
            '渠道访问Token（Slack/Telegram等）': 'token',
            '签名密钥（Slack校验用）': 'signing_secret',
            '路由前缀（如/slack /telegram）': 'prefix',
            '渠道扩展配置（streaming/reply_to_mentions_only等）': 'config',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
    ...
    async def import_template_download_integrations_service(cls) -> bytes:
        ...
        header_list = [
            ...
            '',
            '渠道名称',
            '渠道类型(slack/telegram/whatsapp/agui/discord)',
            '绑定AgentID（三选一）',
            '绑定TeamID（三选一）',
            '绑定WorkflowID（三选一）',
            '渠道访问Token（Slack/Telegram等）',
            '签名密钥（Slack校验用）',
            '路由前缀（如/slack /telegram）',
            '渠道扩展配置（streaming/reply_to_mentions_only等）',
            '',
            '',
            '',
            '',
