...
class AgIntegrationCreateSchema(BaseModel):
    ...
    agent_id: int | None = Field(default=None, description='绑定AgentID（三选一）')
    team_id: int | None = Field(default=None, description='绑定TeamID（三选一）')
    workflow_id: int | None = Field(default=None, description='绑定WorkflowID（三选一）')
    token: str | None = Field(default=None, description='渠道访问Token（Slack/Telegram等）')
    signing_secret: str | None = Field(default=None, description='签名密钥（Slack校验用）')
    prefix: str | None = Field(default=None, description='路由前缀（如/slack /telegram）')
    config: dict | None = Field(default=None, description='渠道扩展配置（streaming/reply_to_mentions_only等）')
