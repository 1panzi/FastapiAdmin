...
class AgWorkflowService:
    ...
    async def list_workflows_service(cls, auth: AuthSchema, search: AgWorkflowQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgWorkflowQueryParam | None - 查询参数
        ...
        obj_list = await AgWorkflowCRUD(auth).list_workflows_crud(search=search_dict, order_by=order_by)
        return [AgWorkflowOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_workflows_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgWorkflowQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgWorkflowCRUD(auth).page_workflows_crud(
            offset=offset,
            limit=page_size,
    ...
    async def create_workflows_service(cls, auth: AuthSchema, data: AgWorkflowCreateSchema) -> dict:
        ...
        - dict - 创建结果
        """
        obj = await AgWorkflowCRUD(auth).create_workflows_crud(data=data)
        if obj and obj.status == "0":
            try:
                get_registry().create_workflow(obj)
    ...
    async def set_available_workflows_service(cls, auth: AuthSchema, data: BatchSetAvailable) -> None:
        ...
        for id in data.ids:
            ...
            if obj:
                obj_list.append(obj)
        await AgWorkflowCRUD(auth).set_available_workflows_crud(ids=data.ids, status=data.status)
        for obj in obj_list:
            try:
                if data.status == "0":
                    get_registry().create_workflow(obj)
    ...
    async def batch_export_workflows_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'stream_executor_events': '是否流式推送执行器事件',
            'store_events': '是否存储事件',
            'store_executor_outputs': '是否存储执行器输出',
            'add_workflow_history_to_steps': '是否将工作流历史传给步骤',
            'num_history_runs': '传给步骤的历史运行次数',
            'add_session_state_to_context': '是否将会话状态加入上下文',
            'debug_mode': '是否开启调试模式',
    ...
    @classmethod
    async def batch_import_workflows_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '是否流式推送执行器事件': 'stream_executor_events',
            '是否存储事件': 'store_events',
            '是否存储执行器输出': 'store_executor_outputs',
            '是否将工作流历史传给步骤': 'add_workflow_history_to_steps',
            '传给步骤的历史运行次数': 'num_history_runs',
            '是否将会话状态加入上下文': 'add_session_state_to_context',
            '是否开启调试模式': 'debug_mode',
            ...
            for _index, row in df.iterrows():
                ...
                try:
                    data = {
                        ...
                        "stream_executor_events": row['stream_executor_events'],
                        "store_events": row['store_events'],
                        "store_executor_outputs": row['store_executor_outputs'],
                        "add_workflow_history_to_steps": row['add_workflow_history_to_steps'],
                        "num_history_runs": row['num_history_runs'],
                        "add_session_state_to_context": row['add_session_state_to_context'],
