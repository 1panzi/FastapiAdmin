...
class AgWorkflowCreateSchema(BaseModel):
    ...
    stream_executor_events: bool = Field(default=..., description='是否流式推送执行器事件')
    store_events: bool = Field(default=..., description='是否存储事件')
    store_executor_outputs: bool = Field(default=..., description='是否存储执行器输出')
    add_workflow_history_to_steps: bool = Field(default=..., description='是否将工作流历史传给步骤')
    num_history_runs: int = Field(default=..., description='传给步骤的历史运行次数')
    add_session_state_to_context: bool = Field(default=..., description='是否将会话状态加入上下文')
