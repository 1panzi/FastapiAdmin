...
class AgSessSummaryConfigService:
    """
    会话摘要配置服务层
    """
    ...
    async def list_sess_summary_configs_service(cls, auth: AuthSchema, search: AgSessSummaryConfigQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgSessSummaryConfigQueryParam | None - 查询参数
        ...
        obj_list = await AgSessSummaryConfigCRUD(auth).list_sess_summary_configs_crud(search=search_dict, order_by=order_by)
        return [AgSessSummaryConfigOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_sess_summary_configs_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgSessSummaryConfigQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgSessSummaryConfigCRUD(auth).page_sess_summary_configs_crud(
            offset=offset,
            limit=page_size,
    ...
    async def create_sess_summary_configs_service(cls, auth: AuthSchema, data: AgSessSummaryConfigCreateSchema) -> dict:
        ...
        - dict - 创建结果
        """
        obj = await AgSessSummaryConfigCRUD(auth).create_sess_summary_configs_crud(data=data)
        return AgSessSummaryConfigOutSchema.model_validate(obj).model_dump()
    ...
    async def set_available_sess_summary_configs_service(cls, auth: AuthSchema, data: BatchSetAvailable) -> None:
        ...
        - None
        """
        await AgSessSummaryConfigCRUD(auth).set_available_sess_summary_configs_crud(ids=data.ids, status=data.status)
    ...
    async def batch_export_sess_summary_configs_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'name': '会话摘要配置名称',
            'model_id': '关联摘要模型ID',
            'session_summary_prompt': '摘要生成提示词',
            'summary_request_message': '摘要请求消息',
            'status': '',
            'description': '',
            'created_time': '',
            'updated_time': '',
    ...
    @classmethod
    async def batch_import_sess_summary_configs_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '会话摘要配置名称': 'name',
            '关联摘要模型ID': 'model_id',
            '摘要生成提示词': 'session_summary_prompt',
            '摘要请求消息': 'summary_request_message',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
