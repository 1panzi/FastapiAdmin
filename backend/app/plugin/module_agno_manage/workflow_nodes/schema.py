...
class AgWorkflowNodeCreateSchema(BaseModel):
    ...
    parent_node_id: int = Field(default=..., description='父节点ID（NULL为顶层节点）')
    node_order: int = Field(default=..., description='节点顺序')
    node_type: str = Field(default=..., description='节点类型(step/condition/loop/parallel/router)')
    name: str = Field(default=..., description='节点名称')
    executor_type: str = Field(default=..., description='执行器类型(agent/team/custom)')
    agent_id: int = Field(default=..., description='联AgentID（executor_type=agent时）')
    team_id: int = Field(default=..., description='关联TeamID（executor_type=team时）')
    executor_module: str = Field(default=..., description='自定义执行器模块路径（executor_type=custom时）')
    add_workflow_history: bool = Field(default=..., description='是否传入工作流历史')
    num_history_runs: int = Field(default=..., description='传入历史运行次数')
    strict_input_validation: bool = Field(default=..., description='是否严格校验输入')
    max_retries: int = Field(default=..., description='最大重试次数')
    skip_on_failure: bool = Field(default=..., description='失败时是否跳过')
    evaluator_type: str = Field(default=..., description='条件评估器类型(bool/cel/function)')
    evaluator_value: str = Field(default=..., description='条件评估器值')
    branch: str = Field(default=..., description='分支标记(if/else)')
    max_iterations: int = Field(default=..., description='循环最大迭代次数')
    end_condition_type: str = Field(default=..., description='循环终止条件类型')
    end_condition_value: str = Field(default=..., description='循环终止条件值')
    forward_iteration_output: bool = Field(default=..., description='是否传递迭代输出')
    selector_type: str = Field(default=..., description='路由选择器类型')
    selector_value: str = Field(default=..., description='路由选择器值')
    allow_multiple_selections: bool = Field(default=..., description='是否允许多路由选择')
    requires_confirmation: bool = Field(default=..., description='是否需要用户确认（HITL）')
    confirmation_message: str = Field(default=..., description='确认提示消息')
    requires_user_input: bool = Field(default=..., description='是否需要用户输入（HITL）')
    user_input_message: str = Field(default=..., description='用户输入提示消息')
    user_input_schema: dict = Field(default=..., description='用户输入结构体Schema')
    on_reject: str = Field(default=..., description='用户拒绝时处理策略(skip/abort)')
    on_error: str = Field(default=..., description='节点出错时处理策略(skip/abort)')
