...
from .schema import AgModelCreateSchema, AgModelOutSchema, AgModelUpdateSchema
...
class AgModelCRUD(CRUDBase[AgModelModel, AgModelCreateSchema, AgModelUpdateSchema]):
    ...
    async def delete_models_crud(self, ids: list[int]) -> None:
        """
        批量删除
    ...
    async def set_available_models_crud(self, ids: list[int], status: str) -> None:
        """
        批量设置可用状态
    ...
    async def page_models_crud(self, offset: int, limit: int, order_by: list[dict] | None = None, search: dict | None = None, preload: list | None = None) -> dict:
        """
        分页查询
