...
from .service import AgModelService
from .schema import AgModelCreateSchema, AgModelUpdateSchema, AgModelQueryParam

AgModelRouter = APIRouter(prefix='/models', tags=["模型管理模块"]) 
...
async def get_models_detail_controller(
    ...
    """
    result_dict = await AgModelService.detail_models_service(auth=auth, id=id)
...
async def get_models_list_controller(
    ...
    search: AgModelQueryParam = Depends(),
    auth: AuthSchema = Depends(AuthPermission(["module_agno_manage:models:query"]))
) -> JSONResponse:
    """
    查询模型管理列表接口（数据库分页）
    ...
    - search: AgModelQueryParam - 查询参数
    - auth: AuthSchema - 认证信息
    
    返回:
    - JSONResponse - 包含模型管理列表的JSON响应
    """
    result_dict = await AgModelService.page_models_service(
        auth=auth,
...
async def create_models_controller(
    ...
    """
    result_dict = await AgModelService.create_models_service(auth=auth, data=data)
    log.info("创建模型管理成功")
...
async def update_models_controller(
    ...
    """
    result_dict = await AgModelService.update_models_service(auth=auth, id=id, data=data)
    log.info("修改模型管理成功")
    return SuccessResponse(data=result_dict, msg="修改模型管理成功")

@AgModelRouter.delete(
    "/delete",
    summary="删除模型管理",
    description="删除模型管理"
)
async def delete_models_controller(
    ...
    """
    await AgModelService.delete_models_service(auth=auth, ids=ids)
...
async def batch_set_available_models_controller(
    ...
    """
    await AgModelService.set_available_models_service(auth=auth, data=data)
...
async def export_models_list_controller(
    ...
    """
    result_dict_list = await AgModelService.list_models_service(search=search, auth=auth)
    export_result = await AgModelService.batch_export_models_service(obj_list=result_dict_list)
    log.info('导出模型管理成功')
...
async def import_models_list_controller(
    ...
    """
    batch_import_result = await AgModelService.batch_import_models_service(file=file, auth=auth, update_support=True)
    log.info("导入模型管理成功")
...
async def export_models_template_controller() -> StreamingResponse:
    ...
    """
    import_template_result = await AgModelService.import_template_download_models_service()
