...
class AgUserRoleService:
    ...
    async def list_user_roles_service(cls, auth: AuthSchema, search: AgUserRoleQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgUserRoleQueryParam | None - 查询参数
        ...
        obj_list = await AgUserRoleCRUD(auth).list_user_roles_crud(search=search_dict, order_by=order_by)
        return [AgUserRoleOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_user_roles_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgUserRoleQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgUserRoleCRUD(auth).page_user_roles_crud(
            offset=offset,
            limit=page_size,
    ...
    @classmethod
    async def batch_import_user_roles_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
