...
class AgTeamMemberService:
    ...
    async def list_team_members_service(cls, auth: AuthSchema, search: AgTeamMemberQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgTeamMemberQueryParam | None - 查询参数
        ...
        obj_list = await AgTeamMemberCRUD(auth).list_team_members_crud(search=search_dict, order_by=order_by)
        return [AgTeamMemberOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_team_members_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgTeamMemberQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgTeamMemberCRUD(auth).page_team_members_crud(
            offset=offset,
            limit=page_size,
    ...
    async def batch_export_team_members_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'uuid': '',
            'team_id': '所属TeamID',
            'member_type': '成员类型(agent/team)',
            'member_id': '成员ID（agent或嵌套team）',
            'role': '成员角色描述',
            'member_order': '成员排序（数字小优先）',
            'status': '',
            'description': '',
            'created_time': '',
            'updated_time': '',
    ...
    @classmethod
    async def batch_import_team_members_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '': 'uuid',
            '所属TeamID': 'team_id',
            '成员类型(agent/team)': 'member_type',
            '成员ID（agent或嵌套team）': 'member_id',
            '成员角色描述': 'role',
            '成员排序（数字小优先）': 'member_order',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
