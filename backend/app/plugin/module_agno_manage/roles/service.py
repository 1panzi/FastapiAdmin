...
class AgRoleService:
    ...
    async def list_roles_service(cls, auth: AuthSchema, search: AgRoleQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgRoleQueryParam | None - 查询参数
        ...
        obj_list = await AgRoleCRUD(auth).list_roles_crud(search=search_dict, order_by=order_by)
        return [AgRoleOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_roles_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgRoleQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgRoleCRUD(auth).page_roles_crud(
            offset=offset,
            limit=page_size,
    ...
    async def batch_export_roles_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            'id': '',
            'uuid': '',
            'name': '角色名称（唯一，如admin/operator/viewer）',
            'scopes': 'gentOS权限范围列表（JSON数组）',
            'status': '',
            'description': '',
            'created_time': '',
            'updated_time': '',
    ...
    @classmethod
    async def batch_import_roles_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            '': 'id',
            '': 'uuid',
            '角色名称（唯一，如admin/operator/viewer）': 'name',
            'gentOS权限范围列表（JSON数组）': 'scopes',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
    ...
    async def import_template_download_roles_service(cls) -> bytes:
        ...
        header_list = [
            '',
            '',
            '角色名称（唯一，如admin/operator/viewer）',
            'gentOS权限范围列表（JSON数组）',
            '',
            '',
            '',
