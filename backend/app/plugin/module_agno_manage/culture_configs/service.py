...
class AgCultureConfigService:
    ...
    async def list_culture_configs_service(cls, auth: AuthSchema, search: AgCultureConfigQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgCultureConfigQueryParam | None - 查询参数
        ...
        obj_list = await AgCultureConfigCRUD(auth).list_culture_configs_crud(search=search_dict, order_by=order_by)
        return [AgCultureConfigOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_culture_configs_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgCultureConfigQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgCultureConfigCRUD(auth).page_culture_configs_crud(
            offset=offset,
            limit=page_size,
    ...
    async def create_culture_configs_service(cls, auth: AuthSchema, data: AgCultureConfigCreateSchema) -> dict:
        ...
        - dict - 创建结果
        """
        obj = await AgCultureConfigCRUD(auth).create_culture_configs_crud(data=data)
    ...
    async def set_available_culture_configs_service(cls, auth: AuthSchema, data: BatchSetAvailable) -> None:
        ...
        for id in data.ids:
            ...
            if obj:
                obj_list.append(obj)
        await AgCultureConfigCRUD(auth).set_available_culture_configs_crud(ids=data.ids, status=data.status)
        for obj in obj_list:
            if data.status == "0":
                get_registry().update_culture_row(str(obj.id), obj)
            else:
    ...
    async def batch_export_culture_configs_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'name': '文化配置名称',
            'model_id': '关联模型ID',
            'add_knowledge': '是否允许新增知识',
            'update_knowledge': '是否允许更新知识',
            'delete_knowledge': '是否允许删除知识',
            'clear_knowledge': '是否允许清空知识',
            'culture_capture_instructions': '文化捕获指令',
    ...
    @classmethod
    async def batch_import_culture_configs_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '文化配置名称': 'name',
            '关联模型ID': 'model_id',
            '是否允许新增知识': 'add_knowledge',
            '是否允许更新知识': 'update_knowledge',
            '是否允许删除知识': 'delete_knowledge',
            '是否允许清空知识': 'clear_knowledge',
            '文化捕获指令': 'culture_capture_instructions',
