...
class AgAgentCreateSchema(BaseModel):
    ...
    reasoning_model_id: int = Field(default=..., description='推理模型ID')
    output_model_id: int = Field(default=..., description='输出模型ID（response_model）')
    parser_model_id: int = Field(default=..., description='解析模型ID')
    memory_manager_id: int = Field(default=..., description='记忆管理器ID')
    learning_config_id: int = Field(default=..., description='学习机配置ID')
    reasoning_config_id: int = Field(default=..., description='推理配置ID')
    compression_config_id: int = Field(default=..., description='压缩管理器配置ID')
    session_summary_config_id: int = Field(default=..., description='会话摘要配置ID')
    culture_config_id: int = Field(default=..., description='文化管理器配置ID')
    instructions: str = Field(default=..., description='Agent指令（system prompt）')
    expected_output: str = Field(default=..., description='期望输出格式说明')
    additional_context: str = Field(default=..., description='附加上下文')
    reasoning: bool = Field(default=..., description='是否开启推理')
    reasoning_min_steps: int = Field(default=..., description='最少推理步数')
    reasoning_max_steps: int = Field(default=..., description='最多推理步数')
    learning: bool = Field(default=..., description='是否开启学习')
    search_knowledge: bool = Field(default=..., description='是否搜索知识库')
    update_knowledge: bool = Field(default=..., description='是否允许更新知识库')
    add_knowledge_to_context: bool = Field(default=..., description='是否将知识库内容加入上下文')
    ...
    add_memories_to_context: bool = Field(default=..., description='是否将记忆加入上下文')
    add_history_to_context: bool = Field(default=..., description='是否将历史记录加入上下文')
    num_history_runs: int = Field(default=..., description='加入上下文的历史运行次数')
    num_history_messages: int = Field(default=..., description='加入上下文的历史消息数')
    ...
    tool_choice: str = Field(default=..., description='工具选择策略(none/auto/specific)')
    output_schema: dict = Field(default=..., description='输出结构体JSON Schema')
    input_schema: dict = Field(default=..., description='输入结构体JSON Schema')
    use_json_mode: bool = Field(default=..., description='是否使用JSON输出模式')
    structured_outputs: bool = Field(default=..., description='是否使用结构化输出')
    parse_response: bool = Field(default=..., description='是否解析响应')
    retries: int = Field(default=..., description='失败重试次数')
    delay_between_retries: int = Field(default=..., description='重试间隔秒数')
    exponential_backoff: bool = Field(default=..., description='是否指数退避重试')
    add_datetime_to_context: bool = Field(default=..., description='是否将当前时间加入上下文')
    add_name_to_context: bool = Field(default=..., description='是否将Agent名称加入上下文')
    compress_tool_results: bool = Field(default=..., description='是否压缩工具结果')
    stream: bool = Field(default=..., description='是否开启流式输出')
    stream_events: bool = Field(default=..., description='是否流式推送事件')
    store_events: bool = Field(default=..., description='是否存储事件')
    markdown: bool = Field(default=..., description='是否输出Markdown格式')
    followups: bool = Field(default=..., description='是否生成追问')
    num_followups: int = Field(default=..., description='追问数量')
    debug_mode: bool = Field(default=..., description='是否开启调试模式')
    debug_level: int = Field(default=..., description='调试级别')
    a2a_enabled: bool = Field(default=..., description='是否对外暴露A2A接口')
    is_remote: bool = Field(default=..., description='是否为远程Agent')
    remote_url: str = Field(default=..., description='远程Agent地址')
    remote_agent_id: str = Field(default=..., description='远程Agent标识符')
