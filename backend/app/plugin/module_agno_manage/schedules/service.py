...
class AgScheduleService:
    ...
    async def list_schedules_service(cls, auth: AuthSchema, search: AgScheduleQueryParam | None = None, order_by: list[dict] | None = None) -> list[dict]:
        ...
        参数:
        - auth: AuthSchema - 认证信息
        - search: AgScheduleQueryParam | None - 查询参数
        ...
        obj_list = await AgScheduleCRUD(auth).list_schedules_crud(search=search_dict, order_by=order_by)
        return [AgScheduleOutSchema.model_validate(obj).model_dump() for obj in obj_list]

    @classmethod
    async def page_schedules_service(cls, auth: AuthSchema, page_no: int, page_size: int, search: AgScheduleQueryParam | None = None, order_by: list[dict] | None = None) -> dict:
        """
        分页查询（数据库分页）
        ...
        result = await AgScheduleCRUD(auth).page_schedules_crud(
            offset=offset,
            limit=page_size,
    ...
    async def batch_export_schedules_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'uuid': '',
            'name': '定时任务名称',
            'agent_id': '触发目标AgentID',
            'team_id': '触发目标TeamID',
            'payload': '触发时传入的消息/参数',
            'cron_expr': 'Cron表达式（如0 9 * * 1-5）',
    ...
    @classmethod
    async def batch_import_schedules_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '': 'uuid',
            '定时任务名称': 'name',
            '触发目标AgentID': 'agent_id',
            '触发目标TeamID': 'team_id',
            '触发时传入的消息/参数': 'payload',
            'Cron表达式（如0 9 * * 1-5）': 'cron_expr',
            '时区（如Asia/Shanghai）': 'timezone',
            '任务超时秒数': 'timeout_seconds',
            '失败最大重试次数': 'max_retries',
            '重试间隔秒数': 'retry_delay_seconds',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
