...
class AgDocumentModel(ModelMixin, UserMixin):
    ...
    content_id: Mapped[str | None] = mapped_column(String(128), nullable=True, comment='Agno contents_db 记录ID')
    metadata_config: Mapped[dict | None] = mapped_column(JSON, nullable=True, comment='文档元数据')
    reader_id: Mapped[int | None] = mapped_column(Integer, nullable=True, comment='处理该文档所用的 Reader ID（关联 ag_readers.id）')
