...
class AgKnowledgeBaseCRUD(CRUDBase[AgKnowledgeBaseModel, AgKnowledgeBaseCreateSchema, AgKnowledgeBaseUpdateSchema]):
    ...
    async def delete_knowledge_bases_crud(self, ids: list[int]) -> None:
        """
        批量删除
    ...
    async def set_available_knowledge_bases_crud(self, ids: list[int], status: str) -> None:
        """
        批量设置可用状态
    ...
    async def page_knowledge_bases_crud(self, offset: int, limit: int, order_by: list[dict] | None = None, search: dict | None = None, preload: list | None = None) -> dict:
        """
        分页查询
