...
class AgHookService:
    ...
    async def create_hooks_service(cls, auth: AuthSchema, data: AgHookCreateSchema) -> dict:
        ...
        if obj and obj.status == "0":
            try:
                get_registry().register_hook(str(obj.id), obj)
    ...
    async def batch_export_hooks_service(cls, obj_list: list[dict]) -> bytes:
        ...
        mapping_dict = {
            ...
            'uuid': '',
            'name': 'Hook名称',
            'hook_type': 'Hook类型(pre/post/tool)',
            'module_path': 'Python模块路径',
            'func_name': '函数名',
            'config': '额外配置参数',
            'run_in_background': '是否后台运行（不阻塞响应）',
            'status': '',
            'description': '',
            'created_time': '',
            'updated_time': '',
    ...
    @classmethod
    async def batch_import_hooks_service(cls, auth: AuthSchema, file: UploadFile, update_support: bool = False) -> str:
        """
        批量导入
        ...
        header_dict = {
            ...
            '': 'uuid',
            'Hook名称': 'name',
            'Hook类型(pre/post/tool)': 'hook_type',
            'Python模块路径': 'module_path',
            '函数名': 'func_name',
            '额外配置参数': 'config',
            '是否后台运行（不阻塞响应）': 'run_in_background',
            '': 'status',
            '': 'description',
            '': 'created_time',
            '': 'updated_time',
