  ...
  <div class="app-container">
    ...
    <el-card class="data-table">
      <template #header>
        ...
        <div v-show="visible" class="search-container">
          ...
          >
            ...
            <el-form-item label="成员角色描述" prop="role">
              <el-input v-model="queryFormData.role" placeholder="请输入成员角色描述" clearable />
            </el-form-item>
            <el-form-item label="成员排序" prop="member_order">
              <el-input v-model="queryFormData.member_order" placeholder="请输入成员排序" clearable />
            </el-form-item>
            ...
            <el-form-item>
              <el-button
                v-hasPerm="['module_agno_manage:team_members:query']"
                type="primary"
                icon="search"
                @click="handleQuery"
              >
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:team_members:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:team_members:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_agno_manage:team_members:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_agno_manage:team_members:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_agno_manage:team_members:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'member_id')?.show"
          label="成员ID"
          prop="member_id"
          min-width="140"
          show-overflow-tooltip
        ...
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'member_order')?.show"
          label="成员排序"
          prop="member_order"
          min-width="140"
          show-overflow-tooltip
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_agno_manage:team_members:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_agno_manage:team_members:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="成员排序" :span="2">
            {{ detailFormData.member_order }}
          </el-descriptions-item>
...
const selectionRows = ref<AgTeamMemberTable[]>([]);
const loading = ref(false);
const isExpand = ref(false);
const isExpandable = ref(true);

// 分页表单
const pageTableData = ref<AgTeamMemberTable[]>([]);
...
const curdContentConfig = {
  permPrefix: "module_agno_manage:team_members",
  cols: exportColumns as any,
  importTemplate: () => AgTeamMemberAPI.downloadTemplateAgTeamMember(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await AgTeamMemberAPI.listAgTeamMember(query);
...
// 详情表单
const detailFormData = ref<AgTeamMemberTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<AgTeamMemberPageQuery>({
  page_no: 1,
  page_size: 10,
  team_id: undefined,
...
// 编辑表单
const formData = reactive<AgTeamMemberForm>({
  id: undefined,
  team_id: undefined,
  member_type: undefined,
  member_id: undefined,
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await AgTeamMemberAPI.listAgTeamMember(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: AgTeamMemberForm = {
  id: undefined,
  team_id: undefined,
  member_type: undefined,
  member_id: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await AgTeamMemberAPI.detailAgTeamMember(id);
    if (type === "detail") {
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await AgTeamMemberAPI.updateAgTeamMember(id, { id, ...submitData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await AgTeamMemberAPI.createAgTeamMember(submitData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await AgTeamMemberAPI.deleteAgTeamMember(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await AgTeamMemberAPI.batchAgTeamMember({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    ...
    const response = await AgTeamMemberAPI.importAgTeamMember(formData);
