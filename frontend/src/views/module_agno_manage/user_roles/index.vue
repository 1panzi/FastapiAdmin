  ...
  <div class="app-container">
    ...
    <el-card class="data-table">
        ...
        <div v-show="visible" class="search-container">
          ...
          >
            ...
            <el-form-item label="角色ID" prop="role_id">
              <el-input v-model="queryFormData.role_id" placeholder="请输入角色ID" clearable />
            </el-form-item>
            ...
            <el-form-item>
              <el-button
                v-hasPerm="['module_agno_manage:user_roles:query']"
                type="primary"
                icon="search"
                @click="handleQuery"
              >
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:user_roles:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:user_roles:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_agno_manage:user_roles:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_agno_manage:user_roles:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_agno_manage:user_roles:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'role_id')?.show"
          label="角色ID"
          prop="role_id"
          min-width="140"
          show-overflow-tooltip
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_agno_manage:user_roles:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_agno_manage:user_roles:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="角色ID" :span="2">
            {{ detailFormData.role_id }}
          </el-descriptions-item>
          <el-descriptions-item label="" :span="2">
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="角色ID" prop="role_id" :required="false">
...
const selectionRows = ref<AgUserRoleTable[]>([]);
const loading = ref(false);
const isExpand = ref(false);
const isExpandable = ref(true);

// 分页表单
const pageTableData = ref<AgUserRoleTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "user_id", label: "用户ID（来自外部Auth系统）", show: true },
  { prop: "role_id", label: "角色ID", show: true },
...
// 导出列（不含选择/序号/操作）
const exportColumns = [
  { prop: "user_id", label: "用户ID（来自外部Auth系统）" },
...
const curdContentConfig = {
  permPrefix: "module_agno_manage:user_roles",
  cols: exportColumns as any,
  importTemplate: () => AgUserRoleAPI.downloadTemplateAgUserRole(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await AgUserRoleAPI.listAgUserRole(query);
...
// 详情表单
const detailFormData = ref<AgUserRoleTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<AgUserRolePageQuery>({
  page_no: 1,
  page_size: 10,
  user_id: undefined,
  role_id: undefined,
...
// 编辑表单
const formData = reactive<AgUserRoleForm>({
  id: undefined,
  user_id: undefined,
  role_id: undefined,
});
...
const rules = reactive({
  ...
  user_id: [{ required: false, message: "请输入用户ID（来自外部Auth系统）", trigger: "blur" }],
  role_id: [{ required: false, message: "请输入角色ID", trigger: "blur" }],
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await AgUserRoleAPI.listAgUserRole(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: AgUserRoleForm = {
  id: undefined,
  user_id: undefined,
  role_id: undefined,
};
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await AgUserRoleAPI.detailAgUserRole(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增AgUserRole";
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await AgUserRoleAPI.updateAgUserRole(id, { id, ...submitData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await AgUserRoleAPI.createAgUserRole(submitData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await AgUserRoleAPI.deleteAgUserRole(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await AgUserRoleAPI.batchAgUserRole({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    ...
    const response = await AgUserRoleAPI.importAgUserRole(formData);
