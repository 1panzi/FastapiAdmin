    ...
    <el-card class="data-table">
      <template #header>
        ...
        <div v-show="visible" class="search-container">
          ...
          >
            ...
            <el-form-item label="触发压缩的Token数阈值" prop="compress_token_limit">
              <el-input v-model="queryFormData.compress_token_limit" placeholder="请输入触发压缩的Token数阈值" clearable />
            </el-form-item>
            <el-form-item label="工具调用压缩指令" prop="compress_tool_call_instructions">
              <el-input v-model="queryFormData.compress_tool_call_instructions" placeholder="请输入工具调用压缩指令" clearable />
            </el-form-item>
            ...
            <el-form-item>
              <el-button
                v-hasPerm="['module_agno_manage:compression_configs:query']"
                type="primary"
                icon="search"
                @click="handleQuery"
              >
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:compression_configs:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:compression_configs:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_agno_manage:compression_configs:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_agno_manage:compression_configs:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_agno_manage:compression_configs:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        <el-table-column
          ...
          label="触发工具结果压缩的条数阈值"
          prop="compress_tool_results_limit"
          min-width="140"
          show-overflow-tooltip
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'compress_token_limit')?.show"
          label="触发压缩的Token数阈值"
          prop="compress_token_limit"
          min-width="140"
          show-overflow-tooltip
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'compress_tool_call_instructions')?.show"
          label="工具调用压缩指令"
          prop="compress_tool_call_instructions"
          min-width="140"
          show-overflow-tooltip
        />
        <el-table-column
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_agno_manage:compression_configs:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_agno_manage:compression_configs:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="触发压缩的Token数阈值" :span="2">
            {{ detailFormData.compress_token_limit }}
          </el-descriptions-item>
          <el-descriptions-item label="工具调用压缩指令" :span="2">
            {{ detailFormData.compress_tool_call_instructions }}
          </el-descriptions-item>
...
import AgCompressionConfigAPI, {
  AgCompressionConfigPageQuery,
...
const selectionRows = ref<AgCompressionConfigTable[]>([]);
const loading = ref(false);
const isExpand = ref(false);
const isExpandable = ref(true);

// 分页表单
const pageTableData = ref<AgCompressionConfigTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "name", label: "压缩配置名称", show: true },
  { prop: "model_id", label: "关联压缩模型ID", show: true },
...
// 导出列（不含选择/序号/操作）
const exportColumns = [
  { prop: "name", label: "压缩配置名称" },
  { prop: "model_id", label: "关联压缩模型ID" },
  { prop: "compress_tool_results_limit", label: "触发工具结果压缩的条数阈值" },
  { prop: "compress_token_limit", label: "触发压缩的Token数阈值" },
  { prop: "compress_tool_call_instructions", label: "工具调用压缩指令" },
...
const curdContentConfig = {
  permPrefix: "module_agno_manage:compression_configs",
  cols: exportColumns as any,
  importTemplate: () => AgCompressionConfigAPI.downloadTemplateAgCompressionConfig(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await AgCompressionConfigAPI.listAgCompressionConfig(query);
...
// 详情表单
const detailFormData = ref<AgCompressionConfigTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<AgCompressionConfigPageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  model_id: undefined,
...
// 编辑表单
const formData = reactive<AgCompressionConfigForm>({
  id: undefined,
  name: undefined,
  model_id: undefined,
  compress_tool_results_limit: undefined,
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await AgCompressionConfigAPI.listAgCompressionConfig(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: AgCompressionConfigForm = {
  id: undefined,
  name: undefined,
  model_id: undefined,
  compress_tool_results_limit: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await AgCompressionConfigAPI.detailAgCompressionConfig(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增AgCompressionConfig";
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await AgCompressionConfigAPI.updateAgCompressionConfig(id, { id, ...submitData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await AgCompressionConfigAPI.createAgCompressionConfig(submitData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await AgCompressionConfigAPI.deleteAgCompressionConfig(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await AgCompressionConfigAPI.batchAgCompressionConfig({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    ...
    const response = await AgCompressionConfigAPI.importAgCompressionConfig(formData);
