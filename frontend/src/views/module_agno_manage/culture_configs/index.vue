    ...
    <el-card class="data-table">
      <template #header>
        ...
        <div v-show="visible" class="search-container">
          ...
          >
            ...
            <el-form-item>
              <el-button
                v-hasPerm="['module_agno_manage:culture_configs:query']"
                type="primary"
                icon="search"
                @click="handleQuery"
              >
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:culture_configs:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:culture_configs:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_agno_manage:culture_configs:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_agno_manage:culture_configs:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_agno_manage:culture_configs:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_agno_manage:culture_configs:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_agno_manage:culture_configs:update']"
              type="primary"
              size="small"
              link
              icon="edit"
...
const selectionRows = ref<AgCultureConfigTable[]>([]);
const loading = ref(false);
const isExpand = ref(false);
const isExpandable = ref(true);

// 分页表单
const pageTableData = ref<AgCultureConfigTable[]>([]);
...
const curdContentConfig = {
  permPrefix: "module_agno_manage:culture_configs",
  cols: exportColumns as any,
  importTemplate: () => AgCultureConfigAPI.downloadTemplateAgCultureConfig(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await AgCultureConfigAPI.listAgCultureConfig(query);
...
// 详情表单
const detailFormData = ref<AgCultureConfigTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<AgCultureConfigPageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  model_id: undefined,
  add_knowledge: undefined,
...
// 编辑表单
const formData = reactive<AgCultureConfigForm>({
  id: undefined,
  name: undefined,
  model_id: undefined,
  add_knowledge: undefined,
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await AgCultureConfigAPI.listAgCultureConfig(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: AgCultureConfigForm = {
  id: undefined,
  name: undefined,
  model_id: undefined,
  add_knowledge: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await AgCultureConfigAPI.detailAgCultureConfig(id);
    if (type === "detail") {
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await AgCultureConfigAPI.updateAgCultureConfig(id, { id, ...submitData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await AgCultureConfigAPI.createAgCultureConfig(submitData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await AgCultureConfigAPI.deleteAgCultureConfig(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await AgCultureConfigAPI.batchAgCultureConfig({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    ...
    const response = await AgCultureConfigAPI.importAgCultureConfig(formData);
