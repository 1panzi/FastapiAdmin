  ...
  <div class="app-container">
    ...
    <el-card class="data-table">
      <template #header>
        ...
        <div v-show="visible" class="search-container">
          ...
          >
            ...
            <el-form-item label="gentOS权限范围列表" prop="scopes">
              <el-input v-model="queryFormData.scopes" placeholder="请输入gentOS权限范围列表" clearable />
            </el-form-item>
            ...
            <el-form-item>
              <el-button
                v-hasPerm="['module_agno_manage:roles:query']"
                type="primary"
                icon="search"
                @click="handleQuery"
              >
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:roles:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_agno_manage:roles:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_agno_manage:roles:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_agno_manage:roles:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_agno_manage:roles:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'scopes')?.show"
          label="gentOS权限范围列表"
          prop="scopes"
          min-width="140"
          show-overflow-tooltip
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_agno_manage:roles:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_agno_manage:roles:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="gentOS权限范围列表" :span="2">
            {{ detailFormData.scopes }}
          </el-descriptions-item>
      ...
      <template v-else>
        ...
        >
          <el-form-item label="角色名称" prop="name" :required="false">
            <el-input v-model="formData.name" placeholder="请输入角色名称" />
          </el-form-item>
          <el-form-item label="gentOS权限范围列表" prop="scopes" :required="false">
            <el-input v-model="formData.scopes" placeholder="请输入gentOS权限范围列表" />
          </el-form-item>
...
const selectionRows = ref<AgRoleTable[]>([]);
const loading = ref(false);
const isExpand = ref(false);
const isExpandable = ref(true);

// 分页表单
const pageTableData = ref<AgRoleTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "name", label: "角色名称（唯一，如admin/operator/viewer）", show: true },
  { prop: "scopes", label: "gentOS权限范围列表（JSON数组）", show: true },
...
// 导出列（不含选择/序号/操作）
const exportColumns = [
  { prop: "name", label: "角色名称（唯一，如admin/operator/viewer）" },
  { prop: "scopes", label: "gentOS权限范围列表（JSON数组）" },
...
const curdContentConfig = {
  permPrefix: "module_agno_manage:roles",
  cols: exportColumns as any,
  importTemplate: () => AgRoleAPI.downloadTemplateAgRole(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await AgRoleAPI.listAgRole(query);
...
// 详情表单
const detailFormData = ref<AgRoleTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<AgRolePageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  scopes: undefined,
...
// 编辑表单
const formData = reactive<AgRoleForm>({
  id: undefined,
  name: undefined,
  scopes: undefined,
  status: "0",
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await AgRoleAPI.listAgRole(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: AgRoleForm = {
  id: undefined,
  name: undefined,
  scopes: undefined,
  status: "0",
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await AgRoleAPI.detailAgRole(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增AgRole";
    formData.id = undefined;
    formData.name = undefined;
    formData.scopes = undefined;
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await AgRoleAPI.updateAgRole(id, { id, ...submitData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await AgRoleAPI.createAgRole(submitData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await AgRoleAPI.deleteAgRole(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await AgRoleAPI.batchAgRole({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    ...
    const response = await AgRoleAPI.importAgRole(formData);
